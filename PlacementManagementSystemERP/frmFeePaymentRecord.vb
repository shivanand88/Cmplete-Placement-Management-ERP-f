...
Public Class frmFeePaymentRecord
    ...
    Private Sub cmbHostelerName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbHostelerName.SelectedIndexChanged
        Try
            ...
            con.Open()
            cmd = New OleDbCommand("SELECT FeePayment.FeePaymentID as [Fee Payment ID ], FeePayment.HostelerID as [Hosteler ID], Hostelers.HostelerName as [Hosteler Name], RoomAllotment.Hostelname as [Hostel Name], RoomAllotment.RoomNo as [Room No], FeePayment.AcadYear as [Academic year],  FeePayment.ServiceCharges as [Service Charges], FeePayment.PayableCharges as [Prev Due Charges], FeePayment.TotalPaid as [Total Paid],  FeePayment.DuePayment as [Present Due],  FeePayment.PaymentDate as [Payment date],  FeePayment.BankChallanNumber as [Bank Challan No],  FeePayment.ExtraCharges as [Extra Charges],  FeePayment.Fine  as [Fine] FROM (Hostelers INNER JOIN RoomAllotment ON Hostelers.[HostelerID] = RoomAllotment.[HostelerID]) INNER JOIN FeePayment ON Hostelers.[HostelerID] = FeePayment.[HostelerID] where  Hostelers.HostelerName='" & cmbHostelerName.Text & "' and FeePayment.AcadYear=RoomAllotment.AcadYear order by HostelerName,PaymentDate desc ", con)
