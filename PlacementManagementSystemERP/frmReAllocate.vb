    ...
    Private Sub btnReAllot_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReAllot.Click
        Try
            ...
            If MessageBox.Show("Are you sure want to re-allocate?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.Yes Then
                ...
                rdr = cmd.ExecuteReader()
                If rdr.Read() Then
                    MessageBox.Show("Hosteler is already check In from " & cmbAcadYear.Text & "CheckBox Out First ", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    cmbAcadYear.Focus()
                    rdr.Close()
                    Exit Sub
                End If
                ...
                rdr = cmd.ExecuteReader()
                If rdr.Read() Then
                    MessageBox.Show("Hosteler is already check out from " & cmbAcadYear.Text & "Please select new academic year", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    cmbAcadYear.Focus()
                    Exit Sub
                    If (rdr IsNot Nothing) Then
